...
AppDataHandler.prototype.handle = function (rxInfoArr, appObj) {
  ...
  return BluebirdPromise
    .all([_this.updateDeviceRouting(optimalRXInfo),
    _this.DeviceInfoRedis.update(uploadDataDevAddr, { FCntUp: newFCnt }),
    // _this.DeviceInfoMysql.increaseFcntup(uploadDataDevAddr, newFCnt),
    uploadToAS(appObj),
...
AppDataHandler.prototype.updateDeviceRouting = function (deviceStatus) {
  ...
  return _this.DeviceInfoRedis.read(deviceStatus.DevAddr).then(function (res) {
    if (res) {
      ...
      let DevAddr = Buffer.from(deviceStatus.DevAddr, 'hex');

      // return _this.DeviceRouting.upsertItem(updateOpts);
      return _this.DeviceInfoRedis.update(DevAddr, updateOpts);
