...
DataConverter.prototype.appDataConverter = function (uplinkDataJson) {
  ...
  if (appFRMPayloadBuf) {
    ...
    return _this.DeviceInfoRedis.read(appDataObject.MACPayload.FHDR.DevAddr, ['FCntUp', 'AppEUI'])
      // return _this.DeviceInfoMysql.readItem(whereOpts, ['FCntUp', 'AppEUI'])
      .then(function (res) {
...
DataConverter.prototype.uplinkDataHandler = function (jsonData) {
  ...
  if (uplinkDataId.equals(Buffer.from([consts.UDP_ID_PUSH_DATA]))) {
    ...
    if (uplinkDataJson.hasOwnProperty('rxpk')) {
        ...
        return _this.deDuplication.handle(joinReqJson)
          .then(function (resArr) {
            ...
            const mongoSavedObj = {
              ...
              data: JSON.parse(JSON.stringify(uplinkDataJson, function (key, value) {
        ...
        return _this.deDuplication.handle(uplinkDataJson)
          ...
          .then(function (uplinkInfo) {   //FIXME
            ...
            if (uplinkInfo && uplinkInfo.appObj) {
              ...
              //   DevAddr: devAddr,
              // };
              // return _this.DeviceInfoMysql.readItem(whereOpts).then((res) => {
              return _this.DeviceInfoRedis.read(devAddr, [AppEUI]).then((res) => {
                const collectionName = consts.MONGO_APPMSGCOLLECTION_PREFIX + res.AppEUI.toString('hex');
                const mongoSavedObj = {
                  DevAddr: devAddr.toString('hex'),
                  msgType: consts.MONGO_SAVEDMSG_TYPE.uplink_msg,
                  createdTime: moment().unix(),
                ...
                .then(function (pubData) {
                  if (pubData) {
                    ...
                    const mongoSavedObj = {
                      ...
                      createdTime: moment().unix(),
                      data: JSON.parse(JSON.stringify(pubData, function (key, value) {
                        return value && value.type === 'Buffer' ? Buffer.from(value.data).toString('hex') : value;
                      })),
                    };
                    return _this.mongooseSave(collectionName, mongoSavedObj);
      ...
      // };
      return _this.gatewayInfoRedis.readUserID(uplinkDataJson.gatewayId).then((res) => {
        ...
        if (res.userID) {
          ...
          const mongoSavedObj = {
            ...
            createdTime: moment().unix(),
            gatewayId: uplinkDataJson.gatewayId.toString('hex'),
            data: JSON.parse(JSON.stringify(uplinkDataJson, function (key, value) {
              return value && value.type === 'Buffer' ? Buffer.from(value.data).toString('hex') : value;
            })),
          };
          ...
          return BluebirdPromise.reject(
            new error(JSON.stringify({
              message: 'No UserID in Reids about the gateway',
              gatewayId: uplinkDataJson.gatewayId.toString('hex'),
            })));
        }
      });
...
DataConverter.prototype.downlinkAppConverter = function (txJson, downlinkJson, uplinkInfo) {
  ...
  outputObject.identifier.writeUInt8(consts.UDP_ID_PULL_RESP, 0);
