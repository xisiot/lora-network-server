    ...
    if (uplinkDataJson.hasOwnProperty('rxpk')) {
      ...
      if (messageType === consts.UNCONFIRMED_DATA_UP ||
        ...
        return _this.deDuplication.handle(uplinkDataJson)
          ...
          .then(function (uplinkInfo) {
            ...
            return this.mysqlConn.DeviceInfo.readItem(whereOpts).then((res) => {
          ...
          .then(function (uplinkInfo) {
            if (uplinkInfo && uplinkInfo.appObj) {
              ...
              let uplinkInfo = uplinkInfo.appObj;
              return _this.downlinkDataHandler.appDataDownlink(uplinkInfo,
                _this.downlinkAppConverter.bind(_this))
                .then(function (pubData) {
                  const collectionName = consts.MONGO_APPMSGCOLLECTION_PREFIX + pubData.appObj.AppEUI;
                  const devAddr = pubData.appObj.MACPayload.FHDR.DevAddr;
                  const mongoSavedObj = {
                    DevAddr: devAddr,
      ...
      return _this.gatewayStatHandler.handle(gatawaySata)
        .then(function () {
          ...
          return this.mysqlConn.GatewayInfo.readItem(whereOpts).then((res) => {
...
DataConverter.prototype.mongooseSave = function (collectionName, mongoSavedObj) {
  ...
  msgModel = mongoose.model(collectionName, mongoSavedSchema);
