    ...
    if (res) {
      ...
      return _this.DeviceRouting.upsertItem(updateOpts)
        .then(() => {
          ...
          return _this.DeviceInfo.readItem(query, attributes);
        })
        .then((res) => {
          ...
          return _this.DeviceRouting.readItem(query, attributes);
        })
        .then((res) => {
          Object.keys(res).forEach((key) => {
            devInfo[key] = res[key];
          ...
          console.log(devInfo);
          return _this.deviceInfo.update(updateOpts.DevAddr, devInfo);
        })
        .then(() => {
          return _this.deviceInfo.read(updateOpts.DevAddr)
            .then((res) => {
