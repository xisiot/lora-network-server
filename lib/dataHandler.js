...
const { consts, MQClient, ERROR } = loraLib;
...
DataHandler.prototype.start = function () {
  ...
  let getHandlers = function (name) {
    let handlers = {
      ...
      [_this.options.mqClient_ns.topics.subFromApplicationServer]: function (msg) {
        _this.log.info({
          label: `Msg from ${_this.options.mqClient_ns.topics.subFromApplicationServer}`,
          message: msg.value,
        });

        let message = msg.value;
        if (!message.hasOwnProperty('DevAddr') || !message.hasOwnProperty('FRMPayload')) {
          return BluebirdPromise.reject(
            new Error(`Invalid message from kafka, Message ${JSON.stringify(message)}`));
        }
        
        return dataConverter.applicationAcceptHandler(msg.value);
      },
