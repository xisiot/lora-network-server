...
DataHandler.prototype.start = function () {
  ...
  /* backend client init */
  let dbModels = this.dbModels = new DbModels(DbClient);
  let redisConn = dbModels.redisConn;
  let mysqlConn = dbModels.mysqlConn;
  let mqClient = this.mqClient = new MqClient(this.options.msgQueue, this.log);
  ...
  let getHandlers = function (name) {
    let handlers = {
      ...
      [_this.options.msgQueue.topics.subFromConnector]: function (msg) {
        _this.log.info({
          label: `Msg from ${_this.options.msgQueue.topics.subFromConnector}`,
          message: msg.value,
        });
      ...
      [_this.options.msgQueue.topics.subFromJoinServer]: function (msg) {
        _this.log.info({
          label: `Msg from ${_this.options.msgQueue.topics.subFromJoinServer}`,
          message: msg.value,
        });
  ...
  return Promise.all([mqClient.connect()]).then(function () {
    ...
    _this.log.info('Listening on Kafka topic : ' + _this.options.msgQueue.consumerGroup.topics);
